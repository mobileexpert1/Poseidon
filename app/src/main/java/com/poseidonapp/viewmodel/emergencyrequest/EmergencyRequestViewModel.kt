package com.poseidonapp.viewmodel.emergencyrequest
...
class EmergencyRequestViewModel: BaseViewModel() {

    var emergencyRequestSuccess = MutableLiveData<EmergencyRequestResponse>()

    fun emergencyRequest(sessionToken:String){
        viewModelScope.launch {
            isLoading.value=true
            try {
                val apiClient= ApiClient.getApiClient()
                val response= ResponseHandler().handleSuccess(
                    apiClient!!.emergencyRequest(sessionToken)
                )
                isLoading.value=false
                val data= response.data

                if(data?.status == true){
                    emergencyRequestSuccess.value = data.let { it }
                }else{
