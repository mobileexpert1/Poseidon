package com.poseidonapp.viewmodel.schedules
...
class SchedulesViewModel:BaseViewModel() {
    var scheduleSuccess= MutableLiveData<ScheduleResponse>()

    fun scheduleRequest(sessionToken:String,filterDate:String){
        viewModelScope.launch {
            isLoading.value=true
            try {
                val apiClient= ApiClient.getApiClient()
                val response= ResponseHandler().handleSuccess(
                    apiClient!!.schedulesRequest(sessionToken,filterDate)
                )
                isLoading.value=false
                val data= response.data

                if(data?.status == true){
                    scheduleSuccess.value = data.let { it }
                }else{
