package com.poseidonapp.viewmodel.safetymeetings
...
class SafetyMeetingsViewModel : BaseViewModel() {
    var safetyMeetingSuccess= MutableLiveData<SafetyMeetingsResponse>()

    fun safetyMeetingRequest(sessionToken:String){
        viewModelScope.launch {
            isLoading.value=true
            try {
                val apiClient= ApiClient.getApiClient()
                val response= ResponseHandler().handleSuccess(
                    apiClient!!.safetyMeetingRequest(sessionToken)
                )
                isLoading.value=false
                val data= response.data

                if(data?.status == true){
                    safetyMeetingSuccess.value = data.let { it }
                }else{
