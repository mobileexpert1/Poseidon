package com.poseidonapp.viewmodel.onRoute
...
class OnRouteViewModel: BaseViewModel() {

    var onRouteSuccess = MutableLiveData<OnRouteResponse>()

    fun onRouteRequest(sessionToken : String,requestId: String) {
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                val response = ResponseHandler().handleSuccess(
                    apiClient.onRouteRequest(sessionToken,requestId)
                )
                isLoading.value = false
                val data= response.data?.body()
                if(data?.status == true){
                    onRouteSuccess.value = response.data.body()
                }else{
