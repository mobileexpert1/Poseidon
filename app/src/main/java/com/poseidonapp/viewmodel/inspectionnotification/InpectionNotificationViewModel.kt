package com.poseidonapp.viewmodel.inspectionnotification
...
class InpectionNotificationViewModel : BaseViewModel() {

    var inspectionNotificationSuccess= MutableLiveData<InspectionNotificationResponse>()

    fun inspectionNotificationRequest(sessionToken:String){
        viewModelScope.launch {
            isLoading.value=true
            try {
                val apiClient= ApiClient.getApiClient()
                val response= ResponseHandler().handleSuccess(
                    apiClient!!.notificationsInspectionRequest(sessionToken)
                )
                isLoading.value=false
                val data= response.data

                if(data?.status == true){
                    inspectionNotificationSuccess.value = data.let { it }
                }else{
