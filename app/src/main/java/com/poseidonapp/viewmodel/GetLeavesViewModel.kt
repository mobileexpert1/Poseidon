...
class GetLeavesViewModel : BaseViewModel() {

    var getLeavesSuccess = MutableLiveData<GetLeavesResponse>()

    fun getLeavesRequest(sessionToken:String){
        viewModelScope.launch {
            isLoading.value=true
            try {
                val apiClient= ApiClient.getApiClient()
                val response= ResponseHandler().handleSuccess(
                    apiClient!!.leavesRequest(sessionToken)
                )
                isLoading.value=false
                val data= response.data

                if(data?.status == true){
                    getLeavesSuccess.value = data.let { it }
                }else{
