package com.poseidonapp.viewmodel.dashboard
...
class DashboardViewModel: BaseViewModel() {
    ...
    var dashboardResponseSuccess = MutableLiveData<DashboardResponse>()


    fun dashboardRequest(sessionToken : String,filterByDate : String) {
        viewModelScope.launch {
            isLoading.value=true
            try {
                val apiClient= ApiClient.getApiClient()!!
                val response= ResponseHandler().handleSuccess(
                    apiClient.dashboardRequest(sessionToken,filterByDate)
                )
                isLoading.value=false
                val data= response.data

                if(data?.status == true){
                    dashboardResponseSuccess.value = data.let { it }
                }else{
