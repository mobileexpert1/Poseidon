package com.poseidonapp.viewmodel.organization
...
class OrganizationViewModel : BaseViewModel() {
    ...
    var organizationsSuccess= MutableLiveData<OrganizationResponse>()

    fun organizationsRequest(sessionToken:String){
        viewModelScope.launch {
            isLoading.value=true
            try {
                val apiClient= ApiClient.getApiClient()
                val response= ResponseHandler().handleSuccess(apiClient!!.organizationsRequest(sessionToken))
                isLoading.value=false
                val data= response.data

                if(data?.status == true){

                    organizationsSuccess.value = data.let { it }
                }else{
